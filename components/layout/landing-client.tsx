...
import { BootSequence } from "../animations/boot-sequence"
...
import type { HeroContent, AboutContent, StackContent, ExperienceContent, ContactContent } from "@/lib/db/content"

export interface LandingClientProps {
  heroData: HeroContent | null
  aboutData: AboutContent | null
  stackData: StackContent | null
  experienceData: ExperienceContent | null
...
export const LandingClient: React.FC<LandingClientProps> = ({
  ...
  if (!bootComplete) {
    // If we have custom boot sequences in firestore, we could pass them to BootSequence,
    // but the BootSequence component might not support it yet.
